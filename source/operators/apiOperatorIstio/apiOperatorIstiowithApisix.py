...
def getIstioIngressStatus(inHandler, name, componentName):
    ...
    APISIX_NAMESPACE = "ingress-apisix"  # Namespace where Apisix Gateway is deployed

    try:
        # Get the Apisix Gateway service by label
        api_response = core_api_instance.list_namespaced_service(APISIX_NAMESPACE, label_selector=APISIX_GATEWAY_LABEL)

        # If no service is found, raise an error
        if len(api_response.items) == 0:
            logWrapper(logging.WARNING, 'getIstioIngressStatus', inHandler, 'api/' + name, componentName, "Can not find", "Apisix Gateway")
        ...
        # Return the loadBalancer details and ports
        response = {'loadBalancer': loadBalancer, 'ports': ports}
        logWrapper(logging.INFO, 'getIstioIngressStatus', inHandler, 'api/' + name, componentName, "Apisix Gateway", "Received ingress status.")
